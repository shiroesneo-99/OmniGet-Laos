...
export async function fetchChangelog(): Promise<string> {
  ...
  try {
    const res = await fetch(
      `https://api.github.com/repos/shiroesneo-99/OmniGet-Laos/releases/tags/v${currentVersion}`,
  ...
  try {
    const res = await fetch(
      "https://api.github.com/repos/shiroesneo-99/OmniGet-Laos/releases/latest",
      { headers: { Accept: "application/vnd.github.v3+json" } }
    );
    if (res.ok) {
      const data = await res.json();
